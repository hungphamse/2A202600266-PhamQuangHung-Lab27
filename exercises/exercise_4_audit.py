    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        a: PRAnalysis = await llm.ainvoke([
            {"role": "system", "content": (
                ...
                "exact file names and sections in the diff where you are uncertain."
            )},
...
async def node_route(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=decision,
        reason=f"Routed to {decision} at confidence={c:.0%}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_human_approval(state):
    ...
    # Audit BEFORE interrupt — reviewer has not yet responded
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason="Waiting for human approval via interrupt()",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision=action,
        reason=state.get("human_feedback") or ("escalation path" if state.get("escalation_answers") else "auto path"),
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=None,
        decision="auto",
        reason=f"High confidence ({a.confidence:.0%}) — no human needed",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    # Audit BEFORE interrupt — reviewer has not yet answered
    await audit(state, AuditEntry(
        agent_id=AGENT_ID,
        action="escalate",
        confidence=a.confidence,
...
async def node_synthesize(state):
    ...
    with console.status("[dim]LLM refining review with reviewer answers...[/dim]"):
        refined: PRAnalysis = await llm.ainvoke([
            {"role": "system", "content": (
                "You are a senior code reviewer. You previously reviewed a PR with low confidence "
                "and asked the author clarifying questions. Using their answers, produce a refined "
                "and more confident structured review."
            )},
            {"role": "user", "content": (
                f"PR Title: {state['pr_title']}\n\n"
                f"Original diff:\n{state['pr_diff']}\n\n"
                f"Initial analysis summary: {state['analysis'].summary}\n\n"
                f"Reviewer Q&A:\n{qa}"
            )},
        ]) # type: ignore
